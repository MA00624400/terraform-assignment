...
resource "aws_autoscaling_group" "terraform-ASG-tf" {
  name		     = "terraform-ASG-tf"
  desired_capacity   = 1
  max_size           = 2
  min_size           = 1
  force_delete       = true
...
resource "aws_lb" "ALB-tf" {
  ...
  tags = {
	name  = "terraform-AppLoadBalancer-tf"
    	Project = "terraform-assignment"
  }
}
